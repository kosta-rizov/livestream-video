...
const Thumbnail = ({ src, fallback, isLive, username }: ThumbnailProps) => {
  ...
  if (!src) {
    content = (
      <div className="bg-background flex flec-col items-center justify-center gap-y-4 h-full w-full transition-transform group-hover:-translate-x-2 group-hover:-translate-y-1 rounded-md ">
        <UserAvatar
          username={username}
          imageUrl={fallback}
          isLive={isLive}
